 ...
 *
 * Disassembly of DSDT.aml, Sun Feb 17 02:51:04 2019
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00012B42 (76610)
 *     Revision         0x02
 *     Checksum         0x75
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "CB-01   "
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "CB-01   ", 0x00000001)
{
     ...
     * disassembly, but only 10 were resolved (12 unresolved). Additional
    ...
    Name (SS3, One)
    Name (SS4, One)
    OperationRegion (GNVS, SystemMemory, 0x8D23D698, 0x0635)
    ...
    OperationRegion (OGNS, SystemMemory, 0x8D23EF18, 0x0000000E)
                    ...
                    Method (_PLD, 0, Serialized)  // _PLD: Physical Location of Device
                    {
                        Name (PLDP, Package (0x01)
                        {
                            Buffer (0x14)
                            {
                                ...
                                /* 0008 */  0xB0, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            ...
            Name (BUF0, ResourceTemplate()
{
    IRQNoFlags() { 0, 8, 11, 15 }
        ...
        Device (MSS2)
        {
            Name (_HID, EisaId ("ETD062F"))  // _HID: Hardware ID
        ...
        Device (MSS3)
        {
            Name (_HID, EisaId ("ETD0630"))  // _HID: Hardware ID
            Name (_CID, Package (0x02)  // _CID: Compatible ID
            {
                EisaId ("ETD0000"), 
                EisaId ("PNP0F13")
...
P8XH (One, 0xAB)
        ...
        If (LOr (LEqual (Arg0, 0x03), LEqual (Arg0, 0x04)))
        {
            ...
            If (LNot (LAnd (LEqual (Arg0, 0x03), \_SB.PCI0.LPCB.EC0.RWAK)))
            {
    ...
    Scope (_SB)
    {
        ...
        Device (EPC)
        {
            ...
            Method (_CRS, 0, NotSerialized)  // _CRS: Current Resource Settings
            {
                CreateQWordField (RBUF, \_SB.EPC._Y17._MIN, EMIN)  // _MIN: Minimum Base Address
                CreateQWordField (RBUF, \_SB.EPC._Y17._MAX, EMAX)  // _MAX: Maximum Base Address
                CreateQWordField (RBUF, \_SB.EPC._Y17._LEN, ELEN)  // _LEN: Length
                Store (\_PR.EMNA, EMIN)
                Store (\_PR.ELNG, ELEN)
    ...
    Scope (_SB.PCI0)
    {
        Device (PDRC)
        {
            ...
            Method (_CRS, 0, Serialized)  // _CRS: Current Resource Settings
            {
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y18._BAS, MBR0)  // _BAS: Base Address
                Store (GMHB (), MBR0)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y19._BAS, DBR0)  // _BAS: Base Address
                Store (GDMB (), DBR0)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y1A._BAS, EBR0)  // _BAS: Base Address
                Store (GEPB (), EBR0)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y1B._BAS, XBR0)  // _BAS: Base Address
                Store (GPCB (), XBR0)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y1B._LEN, XSZ0)  // _LEN: Length
                Store (GPCL (), XSZ0)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y1C._BAS, SXRA)  // _BAS: Base Address
                Store (SXRB, SXRA)
                CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y1C._LEN, SXRL)  // _LEN: Length
                Store (SXRS, SXRL)
                If (LNot (HPTE))
                {
                    CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y1D._BAS, HBAS)  // _BAS: Base Address
                    CreateDWordField (BUF0, \_SB.PCI0.PDRC._Y1D._LEN, HLEN)  // _LEN: Length
                    Store (HPTB, HBAS)
                    Store (0x0400, HLEN)
                }
    ...
    OperationRegion (LGNS, SystemMemory, 0x8D227000, 0x00000005)
    ...
    Scope (\)
    {
        OperationRegion (COMP, SystemMemory, 0x8D246C98, 0x0200)
